<template>
  <div style="display: flex; flex-direction: column">
    <v-card tile>
      <v-img :height="$vuetify.breakpoint.mdAndUp ? $vuetify.breakpoint.height - 72 : ''
        " :src="$vuetify.theme.dark
      ? require('@/assets/images/247.jpg.webp')
      : require('@/assets/images/homelight.jpg.webp')
    ">
        <v-container class="fill-height" :class="$vuetify.breakpoint.lgAndUp ? 'px-16 py-8' : 'py-8 px-4'">
          <v-row class="d-flex align-start" style="justify-content: flex-start">
            <v-col cols="12" sm="6">
              <v-card class="glass-card rounded-xl pa-6">
                <v-card-title>
                  <h1 class="font-weight-light secondary--text" :class="$vuetify.breakpoint.lgAndUp ? 'display-1' : 'text-h5'
                    ">
                ...
                <v-card-text class="pb-0">
                  <p class="text-h6 font-weight-light secondary--text mb-0">
                    👋 Hi, I'm
                    <a target="_blank" :href="!$vuetify.theme.dark
                        ? 'www.linkedin.com/in/bankoleesan'
                        : 'https://twitter.com/theneoncoder'
                      " class="secondary--text font-weight-medium">{{
    !$vuetify.theme.dark ? "Bankole Esan" : "@TheNeonCoder"
  }}</a>. A Fullstack Developer with over 8 years coding
                    experience.
            ...
            <v-col cols="12" sm="6" class="d-flex flex-column justify-space-between" style="align-self: stretch">
              <v-toolbar elevation="0" color="transparent" class="d-flex flex-row-reverse" style="width: 100%"
                :floating="$vuetify.breakpoint.mdAndUp" :class="$vuetify.breakpoint.smAndDown ? 'mb-12 mt-0' : ''">
                <v-card class="
                    d-flex
                    align-center
                    glass-card
              ...
              <v-card class="rounded-xl menu-card glass-card px-2 py-4">
                ...
                <v-autocomplete v-model="select" :loading="loading" id="homeSearch" prepend-inner-icon="mdi-magnify"
                  :items="searchTerms" :item-value="searchTerms.route" :item-text="searchTerms.name" cache-items
                  class="mx-4" flat dark rounded @change="gotoResult" :placeholder="$vuetify.breakpoint.mdAndDown
                ...
                <v-card-actions class="py-0">
                  <v-tooltip v-for="(link, index) in otherLinks" :key="index" top>
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn large :href="link.link" target="_blank" icon class="glass-card mx-2 subactioncard"
                        :class="`${link.class}--text`" v-bind="attrs" v-on="on"><v-icon>{{ link.icon }}</v-icon></v-btn>
                    </template>
                    <span>{{ link.tooltipText }}</span>
                  </v-tooltip>
                  <v-tooltip top>
                    <template v-slot:activator="{ on, attrs }">
                      <v-btn large v-on="on" @click="supdialog = true" class="glass-card mx-2 subactioncard" icon
                        v-bind="attrs"><v-icon>mdi-help</v-icon></v-btn>
                    </template>
            ...
            <v-col cols="12" sm="6" v-for="(item, i) in items" :key="i">
              <v-card class="rounded-xl menu-card" :class="`${item.color}-glass-card`">
                <div :class="$vuetify.breakpoint.lgAndUp ? 'pr-8' : 'pr-4'"
                  class="d-flex flex-no-wrap align-center justify-space-between">
                  <v-avatar v-if="$vuetify.breakpoint.lgAndUp" class="ma-3 pl-4" size="125" rounded="xl">
                  ...
                  <v-btn :to="item.link" :target="['/clients'].includes(item.link) ? '_blank' : ''" x-large
                    class="rounded-xl" :class="$vuetify.theme.dark ? 'glass-card' : 'glass-card'"><v-icon size="32"
                      :color="item.color">{{
                        item.icon
                      }}</v-icon></v-btn>
                </div>
    ...
    <v-dialog scrollable :fullscreen="$vuetify.breakpoint.smAndDown" v-model="supdialog" max-width="500">
      <v-card :tile="$vuetify.breakpoint.smAndDown" :class="$vuetify.breakpoint.smAndDown ? '' : 'rounded-xl'">
        <v-card-title class="headline">
          <v-icon color="primary" class="mr-4">mdi-help</v-icon>

          What I'm up to to lately
          <v-spacer></v-spacer>
          <v-btn :class="$vuetify.theme.dark ? 'morph-dark' : 'morph'" icon
            @click="supdialog = false"><v-icon>mdi-close</v-icon></v-btn>
        </v-card-title>
        <div :class="$vuetify.theme.dark ? 'rainbow-dark' : 'rainbow'" style="width: 100%; height: 2px"></div>
        <v-card-text class="py-0 pl-0" :style="{ maxHeight: $vuetify.breakpoint.mdAndUp ? '390px' : '' }">
          <v-timeline align-top dense>
            <v-timeline-item v-for="(item, i) in currently" :key="i" :icon="item.icon" class="pb-2" fill-dot>
                ...
                <v-expansion-panels flat>
                  <v-expansion-panel class="pl-0">
                    <v-expansion-panel-header class="pl-0">
                      <h2 class="headline font-weight-light mb-0" :class="$vuetify.theme.dark
                            ? 'secondary--text'
                            : 'primary--text'
                          ">
                        {{ item.title }}
                    ...
                    <v-expansion-panel-content class="pl-0 contently">
                      <div class="ma-0 pa-0" v-if="item.list">
                        <div class="ma-0 mb-4 pa-0" v-for="(listItem, j) in item.list" :key="j">
                          <h2 class="ma-0 mb-2 pa-0 font-weight-light">
                            {{ j + 1 }}. {{ listItem.title }}
                          </h2>
                          <v-img v-if="listItem.image" :src="require(`@/assets/images/projectImages/${listItem.image}`)
                            " />

                          <div class="ma-0 mt-1 pa-0 caption" v-html="listItem.description"></div>
                        </div>
                      </div>
